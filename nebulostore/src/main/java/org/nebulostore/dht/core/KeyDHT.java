...
public class KeyDHT implements Serializable {
  ...
  public static final BigInteger COMMUNICATION_KEY = new BigInteger("0");
  public static final BigInteger NONADDRESSING_KEY = new BigInteger("1");
   ...
   */
  public static BigInteger combine(BigInteger firstKey, BigInteger secondKey) {
    ...
    byte[] val1 = firstKey.toByteArray();
    byte[] val2 = secondKey.toByteArray();

    byte[] finalVal = new byte[20];
    for (int i = 0; i < 4 && val1.length - i > 0; ++i) {
      finalVal[3 - i] = val1[i];
    }
    for (int i = 0; i < 16 && val2.length - i > 0; ++i) {
      finalVal[19 - i] = val2[i];
    }
    return new BigInteger(finalVal);
  }

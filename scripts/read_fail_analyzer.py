...
def calcPartialCombsAvail(*args):
    #args = (number of combs, lock, allReads, activityPeriods, nParam, kParam, results succ list, results all reads)
    numberOfCombs = args[0]
    lock = args[1]
    allReads = args[2]
    activityPeriods = args[3]
    nParam = args[4]
    kParam = args[5]
    resSuccList = args[6]
    resAllReadsList = args[7]

    combs = []
    overallSuccNum = 0
    overallReads = 0

    for i in xrange(numberOfCombs):
	combs.append(random.sample(replicators.keys(), nParam))
	...
	print("[" + currentThread().name + "] Num: " + str(i) + "/" + str(numberOfCombs))
	#print("[" + currentThread().name + "] Succ num: " + str(succNum))
	#print("[" + currentThread().name + "] All reads: " + str(len(allReads)))
	print("[" + currentThread().name + "] Subresult: " + "{0:.3f}".format(float(succNum)/len(allReads)))
    lock.acquire()
    resSuccList.append(overallSuccNum)
    resAllReadsList.append(overallReads)
    lock.release()
...
def calcCombinationsAvail(c):
	...
	threads.append(Thread(target = calcPartialCombsAvail, args = (200, lock, allReads, activityPeriods, int(c[0]), int(c[1]), succResults, allReadsResults)))
	threads[-1].start()
    for i in range(0, numberOfThreads):
	threads[i].join()
    
    print("{0:.2f}".format(100 * float(sum(succResults))/sum(allReadsResults)))
...
def calcFanInStats(rootDir):
	...
	for dr in dirs:
	    ...
	    for line in testingLog:
		if re.search(re.compile("RecreateObject.*Received.*objectsSizes_"), line) is not None:
